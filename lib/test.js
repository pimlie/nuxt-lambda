...
const { atob } = require('./utils/options')
...
module.exports = {
  ...
  options: {
    ...
    decode: {
      type: 'boolean',
      description: 'Decode response from base64 to ascii'
    },
    debug: {
      type: 'boolean',
    ...
    benchmark: {
      type: 'boolean',
      description: 'Run in benchmark mode, only output boot / execution times and memory usage in json'
    }
  },
  async run (cmd) {
    if (cmd.argv.debug) {
    ...
    if (cmd.argv.benchmark) {
      console.log = _ => _ // eslint-disable-line no-console
      console.debug = _ => _ // eslint-disable-line no-console
    ...
    if (!cmd.argv.decode) {
      event.headers['Accept-Encoding'] = 'gzip, deflate, br'
    }
    ...
    consola.log(cmd.argv.decode ? atob(res.body) : res.body)
