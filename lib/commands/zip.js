...
async function addFolderToZip (zip, dir, options, createEntry) {
  ...
  let { excludeClientFiles, excludeFiles } = options.lambda
  ...
  if (Array.isArray(excludeFiles)) {
    excludeFiles = entry => !options.lambda.excludeFiles.some(path => entry.startsWith(path))
  } else if (excludeFiles && typeof excludeFiles !== 'function') {
    consola.warn('excludeFiles should be a function or array')
    return
  }
  ...
  for (const file of files) {
    ...
    let addToZip = true
    if (excludeFiles || excludeClientFiles) {
      const buildEntry = path.relative(options.buildDir, file)

      if (excludeClientFiles && buildEntry.startsWith('dist/client/')) {
        addToZip = excludeClientFiles(buildEntry.slice(12), file)
      } else if (excludeFiles && excludeFiles(buildEntry, file)) {
        addToZip = false
      }
    }
...
async function run (cmd, { lambdaPath, options }) {
  ...
  const { path: lambdaZip } = await zipFunction(lambdaPath, distDir, options.lambda.zisi || {})
