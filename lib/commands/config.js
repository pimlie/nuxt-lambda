...
async function run (cmd) {
  ...
  const routerBaseSpecified = userConfig.router && typeof userConfig.router.base === 'string'
  ...
  if (cmd.argv.static || normalizedConfig.lambda.static) {
    normalizedConfig.lambda.static = true

    optimizedOptions.build = {
      ...optimizedOptions.build,
      publicPath
    }

    optimizedOptions.clientDir = path.join(optimizedOptions.buildDir, 'dist', 'client')
  }

  optimizedOptions.render.bundleRenderer.shouldPreload = render.bundleRenderer.shouldPreload.toString()
  optimizedOptions.render.bundleRenderer.shouldPrefetch = render.bundleRenderer.shouldPrefetch.toString()
